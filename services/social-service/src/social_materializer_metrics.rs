...
pub fn render_prometheus() -> String {
    format!(
        "# HELP im_social_postgres_materialization_failures_total Supplemental postgres writes that failed before journal append (write rejected; journal unchanged)\n\
         # TYPE im_social_postgres_materialization_failures_total counter\n\
         im_social_postgres_materialization_failures_total {}\n\
         # HELP im_social_postgres_journal_append_failures_after_materialize_total Journal append failures after successful postgres materialize (compensating rollback attempted)\n\
         # TYPE im_social_postgres_journal_append_failures_after_materialize_total counter\n\
         im_social_postgres_journal_append_failures_after_materialize_total {}\n",
