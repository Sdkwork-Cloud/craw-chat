...
pub fn build_conversation_member_access_gate_from_env(
    ...
    Some(conversation_member_access_gate_from_pool(PostgresJournalPool::from_pool(
        pool,
    )))
}
...
{
    let pool = clone_shared_im_postgres_r2d2_pool()?;
    Some(Arc::new(PostgresAggregateStore::from_pool(PostgresJournalPool::from_pool(
        pool,
    ))))
}
