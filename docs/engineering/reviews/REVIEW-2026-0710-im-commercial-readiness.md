...
| High | Projection tenant isolation | PostgreSQL timeline persistence hard-codes organization `default`; the store port omits organization scope. | Human review: change the public port, define existing-row migration/backfill, then add cross-organization live tests. **[2026-07-11 RESOLVED]** The projection store port now scopes every read/write by `(tenant_id, organization_id)`; the hard-coded `default` organization is removed. See Verified Improvements above. Cross-organization live tests remain a follow-up. |
