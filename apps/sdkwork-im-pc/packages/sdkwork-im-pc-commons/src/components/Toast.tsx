...
export const ToastContainer: React.FC = () => {
  ...
  const topToasts = toasts.filter((item) => item.placement === 'top');
  const bottomRightToasts = toasts.filter((item) => item.placement === 'bottom-right');

  return (
    <>
      <div className="fixed top-10 left-1/2 -translate-x-1/2 z-[9999] flex flex-col gap-2 pointer-events-none">
        <AnimatePresence>
          {topToasts.map((item) => renderToast(item, 'top'))}
        </AnimatePresence>
      </div>
      <div className="fixed bottom-6 right-6 z-[9999] flex max-w-[320px] flex-col items-end gap-2 pointer-events-none">
        <AnimatePresence>
          {bottomRightToasts.map((item) => renderToast(item, 'bottom-right'))}
        </AnimatePresence>
      </div>
    </>
  );
...
export const toast = (message: string, type: ToastType = 'info', options?: ToastOptions): void => {
  if (addToast) {
