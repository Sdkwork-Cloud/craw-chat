...
public class NotificationsApi {
    ...
    public func list(pageSize: Int? = nil, cursor: String? = nil) async throws -> NotificationsListResponse? {
        ...
        return try await client.get(ApiPaths.appendQueryString(ApiPaths.appPath("/notifications"), query), responseType: NotificationsListResponse.self)
    }

    /// Request a notification task
    public func requestsCreate(body: RequestNotification) async throws -> NotificationsRequestsCreateResponse201? {
        return try await client.post(ApiPaths.appPath("/notifications/requests"), body: body, params: nil, headers: nil, contentType: "application/json", responseType: NotificationsRequestsCreateResponse201.self)
    }

    /// Get a notification task
    public func retrieve(notificationId: String) async throws -> NotificationsRetrieveResponse? {
        return try await client.get(ApiPaths.appPath("/notifications/\(serializePathParameter(notificationId, PathParameterSpec(name: "notificationId", style: "simple", explode: false)))"), responseType: NotificationsRetrieveResponse.self)
    }
