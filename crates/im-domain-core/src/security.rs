...
pub enum TenantIsolationError {
    #[error(
        "Tenant mismatch: request tenant '{request}' does not match resource tenant '{resource}'"
    )]
    ...
    #[error(
        "Organization mismatch: request org '{request}' does not match resource org '{resource}'"
    )]
...
impl PermissionValidator {
    ...
    fn default_operation_permissions() -> HashMap<String, Vec<String>> {
        HashMap::from([
            (
                "rtc.session.create".to_string(),
                vec!["rtc.sessions.write".to_string()],
            ),
            (
                "rtc.session.read".to_string(),
                vec!["rtc.sessions.read".to_string()],
            ),
            (
                "rtc.session.update".to_string(),
                vec!["rtc.sessions.write".to_string()],
            ),
            (
                "rtc.session.delete".to_string(),
                vec!["rtc.sessions.write".to_string()],
            ),
            (
                "rtc.signal.post".to_string(),
                vec!["rtc.signals.write".to_string()],
            ),
            (
                "rtc.signal.list".to_string(),
                vec!["rtc.signals.read".to_string()],
            ),
            (
                "rtc.participant.invite".to_string(),
                vec!["rtc.participants.write".to_string()],
            ),
            (
                "rtc.participant.accept".to_string(),
                vec!["rtc.participants.write".to_string()],
            ),
            (
                "rtc.participant.reject".to_string(),
                vec!["rtc.participants.write".to_string()],
            ),
        ])
    }
...
impl SignalReplayProtector {
    ...
    pub fn validate_signal_sequence(
        ...
        self.last_sequences
            .insert(rtc_session_id.to_string(), signal_seq);
    ...
    pub fn clear_session(&self, rtc_session_id: &str) {
        ...
        self.signal_hashes
            .retain(|key, _| !key.starts_with(rtc_session_id));
    }
    ...
    pub fn cleanup_expired(&self) {
        let now = Instant::now();
        self.signal_hashes
            .retain(|_, instant| now.duration_since(*instant) < self.max_hash_age);
    }
}
...
mod tests {
    ...
    fn create_test_context() -> SecurityContext {
        SecurityContext {
            ...
            permission_scope: vec![
                "rtc.sessions.write".to_string(),
                "rtc.signals.write".to_string(),
            ],
    ...
    fn tenant_isolation_cross_tenant_denied() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            TenantIsolationError::TenantMismatch { .. }
        ));
    }
    ...
    fn signal_replay_sequence_validation() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            SignalReplayError::StaleSequence { .. }
        ));
        ...
        assert!(matches!(
            result.unwrap_err(),
            SignalReplayError::DuplicateSignal { .. }
        ));
    }
    ...
    fn signal_replay_session_cleanup() {
        ...
        protector
            .validate_signal_sequence("session_1", 1, "hash_1")
            .unwrap();
        protector
            .validate_signal_sequence("session_1", 2, "hash_2")
            .unwrap();
