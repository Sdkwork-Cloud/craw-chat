...
pub enum IdempotencyError {
    ...
    KeyInUse { key: String, locked_at: String },
    ...
    LockFailed { key: String, retry_after_ms: u64 },

    #[error("distributed lock error: {message}")]
    DistributedLockError { message: String },
}
...
impl IdempotencyKey {
    ...
    pub fn to_storage_key(&self) -> String {
        format!(
            "{}:{}:{}",
            self.tenant_id, self.organization_id, self.request_id
        )
    }
...
impl IdempotencyGuard {
    ...
    pub fn check_and_reserve(&mut self, key: &IdempotencyKey) -> Result<bool, IdempotencyError> {
        ...
        let expires_at = (chrono::Utc::now()
            + chrono::Duration::from_std(self.default_ttl).unwrap())
        .to_rfc3339();
            ...
            match &record.state {
                IdempotencyState::Reserved { reserved_at, .. } => {
                    ...
                    if let Ok(reserved_time) = chrono::DateTime::parse_from_rfc3339(reserved_at) {
                        let elapsed =
                            chrono::Utc::now() - reserved_time.with_timezone(&chrono::Utc);
                        if elapsed
    ...
    pub fn complete(
        ...
        let record =
            self.records
                .get_mut(&storage_key)
                .ok_or_else(|| IdempotencyError::KeyNotFound {
    ...
    pub fn get_cached_response(&self, key: &IdempotencyKey) -> Option<String> {
        ...
        self.records
            .get(&storage_key)
            .and_then(|r| r.cached_response.clone())
    }
