...
pub enum RateLimitError {
    #[error(
        "rate limit exceeded: limit={limit}, current={current}, retry_after={retry_after_ms}ms"
    )]
    Exceeded {
        limit: u32,
        current: u32,
    ...
    BurstExceeded { burst: u32, current: u32 },
...
impl TokenBucket {
    ...
    fn try_consume(&mut self, tokens: u32) -> Result<(), RateLimitError> {
        ...
        if tokens > self.tokens {
            let retry_after_ms =
                ((tokens - self.tokens) as f64 / self.refill_rate as f64 * 1000.0) as u64;
    ...
    pub fn check_rate_with_cost(
        ...
        if let Some(&quota) = self.tenant_quotas.get(tenant_id) {
            ...
            let bucket = self
                .buckets
                .entry(tenant_key)
                .or_insert_with(|| TokenBucket::new(quota, quota / 10, quota / 20));
