...
pub enum LifecycleError {
    ...
    ShutdownTimeout { timeout_ms: u64, remaining: u64 },
    ...
    SignalHandlerError { message: String },
}
...
impl ServiceState {
    ...
    pub fn is_shutting_down(&self) -> bool {
        matches!(
            self,
            ServiceState::Draining | ServiceState::ShuttingDown | ServiceState::Stopped
        )
    }
}
...
impl HealthCheckProbes {
    ...
    pub fn get_health_status(&self) -> ServiceHealthStatus {
        ...
        let liveness_results: Vec<HealthCheckResult> =
            self.liveness_checks.values().map(|check| check()).collect();

        let readiness_results: Vec<HealthCheckResult> = self
            .readiness_checks
            .values()
            .map(|check| check())
            .collect();

        let startup_results: Vec<HealthCheckResult> =
            self.startup_checks.values().map(|check| check()).collect();
...
mod tests {
    ...
    fn health_check_with_custom_check() {
        ...
        probes.register_readiness_check(
            "db",
            Box::new(|| HealthCheckResult {
